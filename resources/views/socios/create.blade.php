                ...
                <form action="{{ route('socios.store') }}" method="POST" id="form-crear-socio">
                    ...
                    <div class="row">
                        <div class="col-md-3 mb-3">
                            ...
                            <input type="number" name="numero_socio" id="numero_socio" class="form-control"
                                value="{{ old('numero_socio') }}" required>
                        </div>
                        <div class="col-md-3 mb-3">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <select name="calle_id" id="calle_id" class="form-select" required>
                                ...
                                <option value="{{ $calle->id }}" {{ old('calle_id')==$calle->id ? 'selected' : '' }}>{{
                                    $calle->nombre }}</option>
                                @endforeach
                            </select>
                        </div>
        ...
        new TomSelect("#calle_id", {
            ...
            create: function (input, callback) {
                ...
                fetch("{{ route('calles.store') }}", {
                    ...
                    .then(response => {
                        if (!response.ok) throw response;
                        // Ya que creamos mediante API pero store devuelve redirect,
                        // Laravel redireccionará y fetch no parseará json.
                        // Será mejor que TomSelect de calles por ahora solo busque usando el resource común,
                        // O si devuelve un json en base al request json. En CalleController no diferenciamos json.
                        // Para simplificar, deshabilitar create en TomSelect para calles si no hay ruta API específica.
                        // Asumiremos que el controlador no retorna JSON para calles. Dejaré create simple.
                        return response.json();
                    })
                    .then(json => {
                        callback({ value: json.id, text: json.nombre });
